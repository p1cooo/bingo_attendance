...
class ApiClient {
  ...
  // --- Auth ---
  async login(usernameOrEmail: string, password: string): Promise<{ user: User; coach_profile?: Coach }> {
    const { email } = await this.request<{ email: string }>('/auth/resolve-login', {
      method: 'POST',
      body: JSON.stringify({ username: usernameOrEmail, email: usernameOrEmail }),
    });
    const credential = await signInWithEmailAndPassword(clientAuth, email, password);
    this.setToken(await credential.user.getIdToken());
    return this.getMe();
  }
  ...
  async getBootstrapStatus(): Promise<{ available: boolean }> {
    return this.request('/auth/bootstrap/status');
  }

  async bootstrapSuperAdmin(idToken: string, name: string): Promise<{ user: User }> {
    return this.request('/auth/bootstrap', { method: 'POST', body: JSON.stringify({ idToken, name }) });
  }
