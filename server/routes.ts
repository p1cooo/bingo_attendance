...
router.get('/auth/bootstrap/status', async (_req, res) => {
  try {
    ...
    return res.json({ available: !bootstrap.exists || bootstrap.data()?.super_admin_initialized !== true });
  } catch {
    return res.status(503).json({ error: 'Bootstrap status is unavailable' });
  }
});

router.post('/auth/bootstrap', async (req, res) => {
  const { idToken, name, bootstrapKey } = req.body;
  const configuredBootstrapKey = process.env.SUPER_ADMIN_BOOTSTRAP_KEY;
  if (!configuredBootstrapKey || bootstrapKey !== configuredBootstrapKey) {
    return res.status(403).json({ error: 'The Super Admin bootstrap key is required' });
  }
  if (!idToken || !name || !String(name).trim()) {
    return res.status(400).json({ error: 'A Firebase ID token and name are required' });
  }

  try {
    const decoded = await adminAuth.verifyIdToken(String(idToken));
    if (!decoded.email) return res.status(400).json({ error: 'The Firebase account must have an email address' });

    const user = await firestore.runTransaction(async (transaction) => {
      const bootstrapRef = firestore.collection('system').doc('bootstrap');
      const bootstrap = await transaction.get(bootstrapRef);
      if (bootstrap.exists && bootstrap.data()?.super_admin_initialized === true) {
      ...
      transaction.set(firestore.collection('users').doc(decoded.uid), superAdmin);
      transaction.set(bootstrapRef, { super_admin_initialized: true, super_admin_uid: decoded.uid, initialized_at: createdAt });
      return superAdmin;
    });
    ...
    if (err?.message === 'BOOTSTRAP_COMPLETE') return res.status(409).json({ error: 'The Super Admin bootstrap has already been completed' });
    return res.status(401).json({ error: 'Unable to complete Super Admin bootstrap' });
  }
});

router.post('/auth/login', (_req, res) => {
  return res.status(410).json({ error: 'Use Firebase Authentication to sign in' });
  /*
...
router.post('/auth/resolve-login', (req, res) => {
  const loginInput = String(req.body?.username || req.body?.email || '').trim();
  const user = db.findUserByLogin(loginInput);
...
router.post('/coaches', authenticateUser, requireAdmin, async (req: AuthenticatedRequest, res: Response) => {
  const { name, email, password, phone, color, color_name, bio } = req.body;

  if (!name || !email || !password) {
    return res.status(400).json({ error: 'Coach name, email, and password are required' });
  }
  ...
  try {
    ...
    await firestore.runTransaction(async (transaction) => {
      transaction.set(firestore.collection('coaches').doc(coachId), newCoach);
      transaction.set(firestore.collection('users').doc(firebaseUid!), newUser);
    });
    firestorePersisted = true;

    db.coaches.set(coachId, newCoach);
    db.users.set(firebaseUid, newUser);
    if (!db.saveToDisk()) {
      db.coaches.delete(coachId);
      db.users.delete(firebaseUid);
      throw new Error('LOCAL_PERSISTENCE_FAILED');
    }
    return res.status(201).json(newCoach);
  } catch (err: any) {
    if (firestorePersisted && firebaseUid) {
      try {
        await firestore.runTransaction(async (transaction) => {
          transaction.delete(firestore.collection('coaches').doc(coachId));
          transaction.delete(firestore.collection('users').doc(firebaseUid!));
        });
      } catch {
        // Best-effort rollback; do not disclose infrastructure details to the client.
      }
    }
    if (firebaseUid) {
      try {
        ...
        // The original provisioning failure is returned without exposing account details.
      }
    }
    return res.status(400).json({ error: 'Unable to create the coach login account' });
  }
});
