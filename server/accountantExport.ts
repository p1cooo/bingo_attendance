...
export async function generateAccountantWorkbook(month: string, coachId?: string, classId?: string): Promise<Buffer> {
  ...
  const detail = workbook.addWorksheet('Attendance Totals'); detail.views = [{ showGridLines: false }]; detail.columns = [{ width: 24 }, { width: 24 }, { width: 18 }, { width: 18 }];
  detail.addRow(['Coach', 'Group class', 'Student', 'Attendances']); detail.getRow(1).font = { bold: true, color: { argb: 'FFFFFFFF' } }; detail.getRow(1).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF334155' } };
  for (const coach of report) { const groups = new Map<string, Map<string, number>>(); for (const session of coach.sessions.filter((s) => s.classType === 'GROUP')) { if (!groups.has(session.className)) groups.set(session.className, new Map()); for (const student of session.studentAttendances.filter((s) => s.attended)) groups.get(session.className)!.set(student.name, (groups.get(session.className)!.get(student.name) || 0) + 1); } for (const [className, counts] of groups) for (const [student, count] of counts) detail.addRow([coach.name, className, student, count]); }
  detail.eachRow((row) => row.alignment = { vertical: 'top', wrapText: true });
...
export async function generateAccountantPdf(month: string, coachId?: string, classId?: string): Promise<Buffer> {
  ...
  return new Promise((resolve, reject) => { const pdf = new PDFDocument({ margin: 42, size: 'A4' }); const chunks: Buffer[] = []; pdf.on('data', (chunk) => chunks.push(chunk)); pdf.on('end', () => resolve(Buffer.concat(chunks))); pdf.on('error', reject); pdf.fontSize(18).fillColor('#0f172a').text(`Academy Teaching Attendance - ${month}`); pdf.moveDown(0.35); pdf.fontSize(9).fillColor('#475569').text('Auditable report: only sessions with recorded attendance. Group totals count Present/Late, including replacements.'); pdf.moveDown(); for (const coach of report) { if (pdf.y > 650) pdf.addPage(); pdf.fontSize(13).fillColor('#0f172a').text(`${coach.name} - Total ${coach.totalHours.toFixed(1)} hours`); const individual = coach.sessions.filter((s) => s.classType === 'INDIVIDUAL'); for (const session of individual) pdf.fontSize(10).fillColor('#111827').text(`${session.className} (Total ${session.durationHours.toFixed(1)} hours)  ${shortDate(session.date)}`); const groups = new Map<string, Map<string, number>>(); for (const session of coach.sessions.filter((s) => s.classType === 'GROUP')) { if (!groups.has(session.className)) groups.set(session.className, new Map()); for (const student of session.studentAttendances.filter((s) => s.attended)) groups.get(session.className)!.set(student.name, (groups.get(session.className)!.get(student.name) || 0) + 1); } for (const [className, counts] of groups) { pdf.moveDown(0.25); pdf.fontSize(11).fillColor('#0f172a').text(className); for (const [student, count] of counts) pdf.fontSize(9).fillColor('#475569').text(`  ${student} X${count}`); } pdf.moveDown(0.75); } pdf.end(); });
}
